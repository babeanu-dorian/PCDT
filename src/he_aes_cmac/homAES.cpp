...
void HomAES::encryptAESkey(vector<Ctxt>& eKey, vector<uint8_t>& aesKey,
			   const PubKey& hePK) const
{
  //OLD: assert(aesKey.length()==16 || aesKey.length()==24 || aesKey.length()==32);
  helib::assertTrue<helib::InvalidArgument>(aesKey.size()==16 || aesKey.size()==24 || aesKey.size()==32,
  ...
  long nRoundKeys = 
    AESKeyExpansion(roundKeySchedule, aesKey.data(), aesKey.size()*8);

  long blocksPerCtxt = ea2.size() / 16;

  // Expand the key-schedule, copying each round key blocksPerCtxt times
  vector<uint8_t> expanded(nRoundKeys*blocksPerCtxt*16);
...
void HomAES::homAESenc(vector<Ctxt>& eData, const vector<Ctxt>& aesKey,
		       const vector<uint8_t> inBytes) const
{
...
void encode4AES(Vec<ZZX>& encData, const vector<uint8_t>& data,
		const EncryptedArrayDerived<PA_GF2>& ea2)
{
  long nBlocks = divc(data.size(),16); // ceil( data.length()/16 )
